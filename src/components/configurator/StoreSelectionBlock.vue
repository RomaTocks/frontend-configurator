<template>
  <v-card style="box-shadow: none" v-if="!loading">
    <v-container>
      ...
      <PriceCard changePath="cpu" v-if="configuration.cpu !== null || configuration.cpu !== undefined" :product="configuration.cpu"/>
      <ConfiguratorError to="cpu" :element="configuration.cpu" :errors="errors.cpu"/>
      <v-row class="pa-4">
        <h2>Материнская плата</h2>
      </v-row>
      <PriceCard changePath="motherboard" v-if="configuration.motherboard !== null || configuration.motherboard !== undefined" :product="configuration.motherboard"/>
      <ConfiguratorError to="motherboard" :element="configuration.motherboard" :errors="errors.motherboard"/>
      <v-row class="pa-4">
        <h2>Оперативная память</h2>
      </v-row>
      <PriceCard changePath="ram" v-if="configuration.ram !== null || configuration.ram !== undefined" :product="configuration.ram"/>
      <ConfiguratorError to="ram" :element="configuration.ram" :errors="errors.ram"/>
      <v-row class="pa-4">
        <h2>Видеокарта</h2>
      </v-row>
      <PriceCard changePath="gpu" v-if="configuration.graphicCard !== null || configuration.graphicCard !== undefined" :product="configuration.graphicCard"/>
      <ConfiguratorError to="gpu" :element="configuration.graphicCard" :errors="errors.gpu"/>
      <v-row class="pa-4">
        <h2>Блок питания</h2>
      </v-row>
      <PriceCard changePath="psu" v-if="configuration.psu !== null || configuration.psu !== undefined" :product="configuration.psu"/>
      <ConfiguratorError to="psu" :element="configuration.psu" :errors="errors.psu"/>
      <v-row class="pa-4">
        <h2>Кулер</h2>
      </v-row>
      <PriceCard changePath="fan" v-if="configuration.fan !== null || configuration.fan !== null" :product="configuration.fan"/>
      <ConfiguratorError to="fan" :element="configuration.fan" :errors="errors.fan"/>
      <v-row class="pa-4">
        <h2>Корпус</h2>
      </v-row>
      <PriceCard changePath="chassis" :change="this.currentChassis" v-if="configuration.chassis !== null" :product="configuration.chassis"/>
...
export default {
name: "StoreSelectionBlock",
  components: {PriceCard, ConfiguratorError},
  data:() => ({
    loading: false,
    errors: {},
    configuration: {
